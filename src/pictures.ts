...
//THIS APP ENCODED FOR THE WINDOW (picture.rs) - A GIF WHERE IT MOVES, PNG WHERE THERE WAS AN ALPHA
//CHANNEL TO KEEP AND JPEG EVERYWHERE ELSE - SO THE EXTENSION IS THE ONE THE BYTES ACTUALLY ARE, AND NOT
//THE ONE THEY ARRIVED UNDER: A .webp THAT IS A JPEG ON THE DISK IS A FILE HALF THE PROGRAMS ON A MACHINE
//WILL NOT OPEN
export function pictureName(image: MessageImage): string
{
    const extension = image.source?.startsWith("data:image/png") ? ".png"
        : image.source?.startsWith("data:image/gif") ? ".gif" : ".jpg";

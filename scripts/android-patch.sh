...
# BEHIND THE HOME BUTTON THROUGH A FOREGROUND SERVICE, THAT THE TWO KOTLIN CLASSES android.rs REACHES
...
# CLASSES AND THE LAUNCHER ARE OVERWRITTEN WITH THE COPIES IN scripts/android/ EITHER WAY.
...
# THE LAUNCHER. `tauri android init` FILLS mipmap-* WITH TAURI'S OWN TEMPLATE ICONS AND KNOWS NOTHING OF
# src-tauri/icons, SO THE SET IN scripts/android/res GOES OVER IT - LEGACY, ROUND, AND THE ADAPTIVE LAYERS
# WITH THE PLATE COLOUR THEY ARE DRAWN ON. IT IS COMMITTED RATHER THAN GENERATED HERE BECAUSE
# scripts/icons.sh WANTS rsvg-convert AND PILLOW, WHICH NO BUILD OF THIS SHOULD HAVE TO HAVE
LAUNCHER="$ROOT/scripts/android/res"

if [ ! -d "$LAUNCHER" ]; then
    echo "android-patch: no launcher icons at $LAUNCHER" >&2
    exit 1
fi
...
rm -f "$PROJECT/app/src/main/res/drawable/ic_launcher_background.xml" \
      "$PROJECT/app/src/main/res/drawable-v24/ic_launcher_foreground.xml"

echo "android-patch: the launcher is the WHY2 mark"

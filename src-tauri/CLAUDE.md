...
`system`, `notice`, `error`) is what the frontend styles on — joins, uploads and server notices are not a
separate channel, they are messages nobody said.
...
`send_input("/channel <name>")`, the disconnect button sends `/exit`, the files modal sends
...
Messages carry no channel field. `App.tsx` keeps `messagesByChannel` and files each incoming message into
...
name (persisted to `client.toml`); `getAnsiColor` in `App.tsx` maps code → hex. Both tables must stay in sync.
...
- Styling is Tailwind v4 (`@import "tailwindcss"` in `src/index.css`, configured with CSS custom properties in
  an `@theme inline` block — there is no `tailwind.config.js`). Use the semantic tokens (`bg-card`,
  `text-muted-foreground`, `border-border`), not raw colors. The app is hardcoded dark via a `dark` class on
  the root `<main>`.
